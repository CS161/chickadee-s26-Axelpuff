...
__always_inline backtrace_regs backtrace_proc_regs(const proc* p) {
    if (p->yields_
        && p->contains(p->yields_)
        && p->contains(reinterpret_cast<uintptr_t>(p->yields_ + 1) + 8)) {
        uintptr_t rip = *reinterpret_cast<uintptr_t*>(p->yields_ + 1);
        uintptr_t rsp = reinterpret_cast<uintptr_t>(p->yields_ + 1) + 8;
        return backtrace_regs(rip, rsp, p->yields_->reg_rbp);
    }
    if (p->regs_
        && p->contains(p->regs_)) {
        return backtrace_regs(*p->regs_);
    }
    return backtrace_regs(0, 0, 0);
}
}
...
void assert_fail(const char* file, int line, const char* msg,
    ...
    print_backtrace(pr, br, pt ? pt : backtrace_current_pagetable(), false);
    pr.move_cursor();
    fail();
}
