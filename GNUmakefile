...
$(OBJDIR)/%.ko: %.cc $(KERNELBUILDSTAMPS)
	$(call cxxcompile,$(O) $(KERNELCXXFLAGS) $(DEBUGFLAGS) -DCHICKADEE_KERNEL -mcmodel=kernel -c $< -o $@,COMPILE $<)
...
$(OBJDIR)/k-initfs.ko: $(OBJDIR)/k-initfs.cc
	$(call cxxcompile,$(KERNELCXXFLAGS) $(DEBUGFLAGS) -O2 -DCHICKADEE_KERNEL -mcmodel=kernel -c $< -o $@,COMPILE $<)
